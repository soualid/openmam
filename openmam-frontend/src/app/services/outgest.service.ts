...
export class OutgestService {
  ...
  triggerOutgestTask(outgestProfileId:number, mediaId:number, destinationLocationId: number, 
    videoStreamIds:number[], audioStreamIds:number[]): Observable<Task> {
    ...
    return this.http.post<Task>(url, {
      ...
      .pipe(
        catchError(this.handleError<Task>(`triggerOutgestTask`))
      );
  ...
  getOutgestProfiles(): Observable<Page<OutgestProfile>> {
    return this.http.get<Page<OutgestProfile>>(`${this.outgestUrl}/profiles`)
      .pipe(
        ...
        catchError(this.handleError<Page<OutgestProfile>>('locations', {}))
      );
  }
